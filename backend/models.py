...
class Alert(db.Model):
    ...
    # ML-enhanced fields
    ml_confidence = db.Column(db.Float)  # Enhanced ML confidence score
    temporal_consistency = db.Column(db.Float)  # Temporal consistency score
    false_positive_score = db.Column(db.Float)  # False positive probability
    priority = db.Column(db.Integer, default=100)  # Numeric priority for routing
    ...
    notification_channels = db.Column(db.JSON)  # List of channels used
    notification_status = db.Column(db.JSON)  # Delivery status per channel
